    ...
    <div className="min-h-screen bg-background text-bone flex flex-col justify-center items-center p-4">
      <div className="max-w-md w-full glass p-8 rounded-3xl border border-gold/15 space-y-6 text-center shadow-2xl relative overflow-hidden">
        ...
        <header className="space-y-2">
          ...
          <h1 className="font-display text-2xl text-gold tracking-wide">微信戏命师已绑定</h1>
          <p className="text-xs text-muted-foreground">戏命师已成功入驻你的微信聊天列表</p>
        </header>

        <section className="bg-secondary/40 border border-gold/10 rounded-2xl p-5 text-left space-y-3">
          <h2 className="text-sm font-semibold text-gold">现在回到微信中，发送以下指令：</h2>
          <ul className="space-y-2 text-xs text-muted-foreground font-mono">
            <li>
              🍀「<span className="text-gold">今日</span>」— 查看今日流日与避忌
            </li>
            <li>
              📅「<span className="text-gold">八字起盘</span>」— 输入生日起紫微命盘
            </li>
            <li>
              💰「<span className="text-gold">财运</span>」— 了解今日财富方位
            </li>
            <li>
              🎴「<span className="text-gold">盲盒</span>」— 抽取稀有命师与铭文
            </li>
            <li>
              🏮「<span className="text-gold">命铺</span>」— 开通经营者推广权益
            </li>
          </ul>
        </section>
